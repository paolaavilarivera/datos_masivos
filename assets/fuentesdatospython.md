...
| **Bases de datos SQL (MySQL, PostgreSQL, SQLite, etc.)**           | Sistemas relacionales donde los datos se organizan en tablas con relaciones definidas.                     | Consultas estructuradas, análisis de grandes volúmenes transaccionales, integración con sistemas de negocio. |
| **Bases NoSQL (MongoDB, Cassandra, Redis, etc.)**                  | Bases orientadas a documentos, clave–valor o grafos. Diseñadas para datos no estructurados o distribuidos. | Análisis de logs, datos semiestructurados (JSON), telemetría, grandes volúmenes distribuidos.                |
